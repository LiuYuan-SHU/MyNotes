...
一个指针和引用之所以支持多态，是因为它们并不引发内存中任何“与类型有关的内存委托操作（type-defined commitment）”；会受到改变的，只有它们所指向的内存的“大小和内容解释方式”而已。

但是，当一个base class object直接被初始化为（或者被指定为）一个derived class object的时候，derived object就会被切割（sliced）以塞入较小的基类对象内存中，被派生的内容将不会有任何的痕迹。多态于是不再呈现。

如果我们将virtual function定义为inline，那么会有更大的效率上的收获。

******

总而言之，多台是一种威力强大的设计机制，其付出的代价就是额外的间接性——不论是在“内存的获得”还是在“类型的决断”上。C++通过类的指针和引用来支持多态，这种程序设计风格就被称为“面向对象”。

C++也支持具体的ATD程序风格，如今被称为object-based（OB）。例如string类，一种非多态的数据类型。它提供pulic的接口，private的数据成员，包括了数据和算法，但是并不支持类型的扩充。一个OB设计可能比一个对等的OO设计速度更快而且空间更加紧凑。速度快是因为所有的函数调用操作都在编译时期解析完成，对象建构起来时不需要设置virtual机制；空间紧凑则是因为每一个class object不需要负担传统上为了支持virtual机制而需要的额外负荷。不过，OB设计在弹性上就稍微差一些。

弹性（OO）和效率（OB）之间长长存在着取舍。一个人能够有效选择其一之前，必须先清楚两者的行为和应用领域的需求。
