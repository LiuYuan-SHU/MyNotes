...
对于内联函数的效果和效率问题，不在此赘述。

实际上我们并不能够强迫将任何函数都变成inline。关键词inline只是一项请求。如果这项请求被接受，编译器就必须认为它可以用一个表达式（expression）合理地将这个函数扩展开来。

当我们说“编译器相信他可以合理地扩展一个inline函数”时，我们相信在某种程度上，其执行成本比一般的函数调用及返回机制所所带来的负荷低。

一般而言，处理一个内联函数，有两个阶段：

1. 分析函数定义，以决定函数的“instrinsic inline ability”（本质的inline能力）。

    此处的“本质”的含义是指“与编译器相关”的意思。如果函数因为其复杂度，或者因为其构建问题，被被判断不可成为inline，它就会被转换为一个静态函数（一个常见的函数），并在“被编译模块”内产生对应的函数定义。

2. 真正的内联函数扩展操作是在调用的那一点上。这会带来参数的求值操作（evaluation）以及临时性对象的管理。

### 形式参数（Formal Arguments）

在inline扩展期间，每一个形式残水都会被对应的实际参数取代。如果说有什么副作用，那就是，我们不能只是简单地一一把用到的每一个形参放到对应的位置，因为浙江导致对于实际参数的多次求值操作（evaluations）。一般而言，面对“会带来副作用的实际参数”，通常都需要引入临时性对象。换句话说，如果实际参数是一个常量表达式（constant expression），我们可以在替换之前完成其求值操作（evaluations）；后继的inline替换，就只是简单的把常量绑上去罢了。如果既不是个常量表达式，也不是个带有副作用的表达式，那么直接替换之。
...
(3)将会引发参数的副作用，它需要导入一个临时对象，以避免重复求值（multiple evaluations）：
...
如果我们在inline定义中加入一个局部变量，会怎样？

如果一个内联函数中含有局部变量，在运行内联函数的代码之前，栈大小会扩张一些，以存放内联函数的局部变量，在内联函数代码部分之后，`sp`将会减去局部变量的值。因此，内联函数中的局部变量，再加上有副作用的参数，可能会导致大量临时性对象的产生。因此，如果一个内联函数被调用太多次，就会产生大量的扩展码，使程序大小暴涨。
