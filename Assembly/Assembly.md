...
这种特殊的寄存器在8086CPU中，被称为标志寄存器，我们简称为flag。8086CPU的标志寄存器有16位，其中存储的信息通常被称为程序状态字（PSW）。
...
**注意，在8086CPU的指令集中，有的指令的执行是影响标志寄存器的。比如，`add`/`sub`/`mul`/`div`/`inc`/`or`/`and`等。他们大多是运算指令（进行逻辑或算数运算）；有的指令的执行对标志寄存器没有影响，比如`mov`/`push`/`pop`等，他们大多是传送指令**

### 11.1 ZF标志

flag的第六位是ZF，零标志位。它记录相关指令执行后，其结果是否为0。如果结果位0，那么zf = 1；否则为0.
...
flag的第二位是PF，奇偶标志位。它记录相关指令执行后，其结果的所有bit位中1的个数是否为偶数。如果1的个数为偶数，pf=1；否则为0.
...
执行后，结果为`00001011B`，1的个数为3， 所以pf = 0

### 11.3 SF标志

flag的第七位，符号标志位。它记录相关指令执行后，其结果是否为负。如果结果为负，sf=1.

计算机通常用补码来表示有符号数句。计算机中的一个数据可以看做是有符号数，也可以看成是无符号数。比如：

+ `00000001B`，可以看做是无符号数1，或者有符号数+1
+ `10000001B`，可以看做无符号数129， 也可以看作是有符号数-127

也就是说，对于同一个二进制数据，计算机可以将它当做无符号数据来运算，也可以当做有符号数据来运算，比如：
...
可以将add指令进行的运算当做是无符号数的运算，那么add指令相当于计算129+1，结果为130；也可以将add进行的运算当做是有符号数的运算，那么其相当于$-127+1 = -126$

不管我们如何看待，CPU在执行add等指令的时候，就已经包含了两种含义，也将得到用同一种信息来表示的两种结果。关键在于，我们想要哪一种结果。

SF标志，就是CPU对有符号数运算结果的一种记录，它记录数据的正负。在我们将数据当做有符号数来运算的时候，可以通过它来的值结果的正负。如果我们将数据当做无符号数来运算，SF的值则没有意义，虽然相关的指令影响了它的值。

这也就是说，CPU在执行add等指令时，是必然要影响到SF标志位的值的，至于我们需不需要这种影响，那就看我们如何看待指令所进行的运算了。

## 第十二章 内中断

任何一个通用的一个CPU，比如8086，都具备一种能力，可以在执行完当前正在执行的指令之后，检测到从CPU外部发送过来的或内部产生的一种特殊信息，并且可以理解对所接受到的信息进行处理。这种特殊的信息，我们可以称其为：中断信息。中断的意思是指，CPU不再接着（刚执行完的指令）向下执行，而是转去处理这个特殊信息

注意，我们这里所说的中断信息，是为了便于理解而采用的一种逻辑上的说法。它是对几个具有先后顺序的硬件操作所产生的事件的统一描述。

### 12.1 内中断的产生

当CPU的内部有以下事情发生的时候，将产生需要马上处理的中断信息：
...
上述是不同的中断，自然产生不同的中断信息。既然是不同的中断信息，我们的处理方式自然也就不同。CPU首先需要知道的是，所接受到的中断信息的来源。所以中断信息中必须包含识别来源的编码。8086CPU用称为中断类型码的数据来标识中断信息的来源。中断类型码为一个字节型数据，可以标识256种中断信息的来源。上述的4种中断源，在CPU中的中断类型码如下：
...
CPU在收到中断信息后，应该转区执行该中断信息的处理程序。我们知道，若要8086CPU执行某处的程序，就要将CS:IP指向它的入口。可见首要的问题是，***CPU在收到中断信息后，如何根据中断信息确定其处理程序的入口。***

CPU的设计者必须在中断信息和其处理程序的入口地址之间建立某种联系，使得CPU根据中断信息可以找到要执行的处理程序。

### 12.3 中断向量表

CPU用8位的中断类型码通过中断向量表找到相应的中断处理程序的入口地址。那么什么是中断向量表呢？中断向量表哦就是中断向量的列表。那么什么又是中断向量呢？所谓中断向量，就是中断处理程序的入口地址。展开来讲，中断向量表，就是中断处理程序入口地址的列表。

终端向量表在内存中保存，其中存放这256个中断源所对应的中断处理程序的入口。

<img src="Assembly.assets/image-20221025191055621.png" alt="image-20221025191055621" style="zoom:50%;" />

CPU只要知道了中断类型码，就可以将中断类型码作为中断向量表的表项号，定位相应的表项，从而得到中断处理程序的入口地址。

可见，CPU用中断类型码，通过查找中断向量表，就可以得到中断处理程序的入口地址。在这个方案中，一个首要的问题就是，CPU如何找到中断向量表？

中断向量表在内存中存放，***对于8086PC机，中断向量表指定放在内存地址0处。***内存0000:0000到0000:03FF的1024个单元中存放着中断向量表。

我们已经知道我们一共有256个中断，中断向量表占用了1024个字节，也就是说，一个中断占用了4个字节的空间。***高地址的字存放段地址，低地址的字存放偏移地址。***

### 12.4 中断过程

用中断类型吗找到中断向量，并用它设置CS和IP，这个工作是由CPU的硬件自动完成的，CPU硬件完成这个工作的过程被称为中断过程。

CPU收到中断信息后，要对中断信息进行处理，首先将引发终端过程。硬件在完成中断过程后，CS:IP将指向中断处理程序的入口，CPU开始执行中断处理程序。

有一个问题需要考虑，CPU在执行完中断处理程序之后，应该返回原来的执行点继续执行下面的指令。所以我们在调用中断程序之前需要先保存现在的CS:IP，然后再去设置新的CS:IP。
...
4. CS入栈
5. IP入栈
6. 从内存地址为中断类型码 * 4和中断类型码 * 4 + 2的两个字单元中读取中断处理程序的入口地址，设置CS和IP
...
> 编程：当发生除法移除时，在屏幕中间显示"overflow!"，返回DOS

假设我们能够写出中断程序，我们首先要考虑的第一个问题是：我们编写的程序应该放在内存中。因为除法溢出随时可能发生，CPU随时都可能将CS：IP指向程序入口，执行程序。

那么我们的程序应该放在哪里呢？

我们在此不去讨论如何向计算机请求资源，我们只需直接找到一块别的程序不会用到的内存区，将我们编写的代码传送到此处即可。

前面讲到，内存0000:0000~0000:03FF都是中断向量表。也就是说8086CPU支持256个终端，但是我们远远没有那么多向量。也就是说，在中断向量表中，有很多位置是空的。

一般情况下，从0000:0200到0000:02FF的这段空间都是空的，操作系统和其他应用程序都不占用。

然后我们再去修改中断向量表，将我们的程序的地址存放到中断向量表中。
...
程序12.1开始执行的时候，do0的代码是不执行的，它只是作为do0安装程序所要传送的数据。程序12.1执行的时候，首先执行do0安装程序，然后将do0的入口地址保存在中断向量表。然后程序12.1就结束了。程序的目的就是在内存0:200处安装do0的代码，将0号中断的程序入口更改。

### 12.8 安装——`movsb`

我们可以使用[`movsb`](https://baike.baidu.com/item/MOVSB/7161770?fr=aladdin)指令，来将我们编写的do0代码送入0:200。

同时再使用[`cld`](https://blog.csdn.net/qwq1503/article/details/102984575)指令来设置递增。
...
对于上面我们使用文字描述的部分，首先的问题是，我们如何知道do0代码的长度？最简单的方法是，计算一下do0中所有指令码的字节数。但是这样太麻烦了，因为我们只要修改do0的内容，我们就需要重新计算他的长度。

我们可以使用编译器来计算do0的长度，具体做法如下：

```assembly
mov cx, offset do0end - offset do0
```

`-`是编译器识别的运算符号，编译器可以用它来进行两个常数的减法。同样`+`/`*`/`/`也都是可以处理的。

### 12.9 do0程序

在编写do0程序的时候，我们需要考虑的问题是，do0所打印的字符串的在哪？如果我们将其存放在安装程序中，当安装程序结束，操作系统回收资源之后，我们的字符串便不复存在。因此这样是不可行的，我们唯一的方法就是将其放在代码段中，然后一起写入do0的目的地址。
...
code segment
	start:	mov ax, cs
			...
			mov cx, offset do0end-offset do0	; 设置传输字长
			cld					; 设置传输方向
			rep movsp			; 串操作
			...
			; do0程序从这里开始被复制到0:200
	do0:	jmp short do0start					; 本指令占两个字节
			db "overflow!"
	do0start:	mov ax, cs
				mov ds, ax
				...
				mov ax, 0b800h
				mov es, ax
				mov di, 12 * 160 + 36 * 2		; 设置es:di指向显存空间的中间
...
在系统板的ROM中存放着一套程序，称为BIOS（Basic IO System），BIOS中主要包含以下几部分内容：
...
BIOS和DOS在所提供的中断例程中包含了许多子程序，这些子程序实现了程序员在编程的时候经常需要用到的功能。程序员在编程的时候，可以用`int`指令直接调用BIOS和DOS提供的中断例程，来实现某些工作。

和硬件设备相关的DOS中断例程中，一般都调用了BIOS的中断例程。

### 13.5 BIOS和DOS中断例程的安装过程

1. 开机后，CPU加电，初始化(CS)=0FFFFH, (IP) = 0，自动从FFFF:0单元开始执行程序。FFFF:0单元开始执行程序。FFFF:0处有一条跳转指令，CPU执行该指令后，转去执行BIOS中的硬件系统检测和初始化程序
2. 初始化程序将建立BIOS所支持的中断向量，即，将BIOS提供的终端历程的入口地址登记在中断向量表中。注意，对于BIOS所提供的中断例程，只需将入口地址登记在中断向量表中即可，因为它们是固化在ROM中的程序，在内存中一直存在
...
1. 各种接口卡（网卡、显卡等）上的接口芯片，它们控制接口卡进行工作
...
可见，从CPU的角度来说，将这些寄存器都当做端口，对它们进行统一编址，从而建立了一个统一的端口地址空间。每一个端口在地址空间中都有一个地址。
...
out 20h, al		; 往20h号端口放入一个字节
```



***注意：在`in`和`out`指令中，我们只可以使用`ax`或者`al`来存放从端口中读入的数据或要发送到端口中的数据。访问8位端口时用`al`， 访问16位端口时用`ax`***

### 14.2 CMOS RAM芯片

> 下面的内容中，我们通过对CMOS RAM的读写来体会一下端口的访问

PC机中，有一个CMOS RAM芯片，一般简称为CMOS。此芯片的特征如下：

1. 包含一个实时钟和一个有128个存储单元的RAM存储器（早期的计算机为64个自己）
2. 该芯片靠电池供电。所以，关机后其内部的实时钟仍可正常工作，RAM中的信息不丢失
3. 128个字节的RAM中，内部实时钟占用0~0dh单元来保存时间信息，其余大部分单元用于保存系统配置信息，供系统启动时BIOS程序读取。BIOS也提供了相关的程序，是我们可以在开机的时候配置CMOS RAM中的系统信息
4. 该芯片内部有两个端口，端口地址为70h和71h。CPU通过这两个端口来读写CMOS RAM
5. 70h为地址端口，存放要访问的CMOS RAM单元的地址；71h为数据端口，存放从选定的CMOS RAM单元中读取的数据，或要写入到其中的数据。可见，CPU对CMOS RAM的读写分为两步进行。比如，度CMOS RAM的2号单元：
    1. 将2送入端口70h
    2. 从端口71h读出2号单元的内容
...
***注意：如果移动位数大于1， 必须将移动的位数放在cl中***

******

shr是逻辑右指令，它和shl所进行的操作刚好相反

1. 将一个寄存或内存单元中的数据向右移位
2. 将最后移出的一位写入CF中
3. 最高位用0补充
...
> CPU在计算机系统中，除了能够执行指令，进行运行意外，还应该能够对外部设备进行控制，接收它们的输入，向他们进行输出。也就是说，CPU除了有运算能力外，还要有IO能力。比如，我们按下键盘上的一个键，CPU最终要能够处理这个件。在使用文本编辑器时，按下a键后，我们可以看到屏幕上出现“a”，是CPU将从键盘上输入的键所对应的字符送到显示器上的
...
在之前，我们讨论过，PC系统的接口卡和主板上，装有各种接口芯片。这些外设接口芯片的内部有若干寄存器，CPU将这些寄存器当做端口来访问。

外设的输入不直接送入内存和CPU，而是送入相关的接口芯片中；CPU向外设的输出也不是直接送入外设，而是先送入端口中，再由相关的芯片送到外设。CPU还可以向外设输出控制命令，而这些控制命令也是先送到相关芯片的端口中，然后再由相关的芯片根据命令对外设实施控制，

***可见，CPU通过端口和外部设备进行联系。***

### 15.2 外中断信息

现在，我们知道了外设的输入被存放在端口中，可是外设的输入随时都有可能到达，CPU如何及时的知道，并进行处理呢？更一般的讲，就是外设随时都可能发生需要CPU及时处理的时间，CPU如何及时得知并进行处理？

CPU提供中断机制来满足这种需要。

1. 前面讲过，当COU的内部有需要处理的事情发生的时候，将产生中断信息，引发中断过程。这种中断信息来自于CPU内部。
2. 还有一种中断信息，来自于CPU外部，当CPU外部有需要处理的事情发生的时候，比如说，外设的输入到达，相关芯片将向CPU发出相应的中断信息。CPU在执行完当前指令后，可以检测到发送过来的中断信息，引发中断过程，处理外设的输入。

#### 1. 可屏蔽中断

可屏蔽中断是CPU可以不响应的中断。CPU是否响应可屏蔽中断，要看标志寄存器的IF位的设置。当CPU检测到可屏蔽中断时，如果IF = 1，则CPU在执行完当前指令后响应中断，引发中断过程；如果IF = 0， 则不响应可屏蔽中断。
...
可屏蔽中断所引发的中断过程，除了在第一步的实现上有所不同，基本上和内中断的中断过程仙童。因为可屏蔽你中断信息来自于CPU外部，中断类型码是通过数据总线送入CPU的；而内中断的中断类型码实在CPU内产生的。

***现在，我们可以解释中断过程中将IF置为0 的原因了。将IF置0的原因解释，在进入中断处理程序后，禁止其他的可屏蔽中断。***

当然，如果在终端处理程序中需要处理可屏蔽中断，我们可以用指令将IF置为1。
...
不可屏蔽中断是CPU必须响应的外中断。当CPU检测到不可屏蔽中断的时候，则在执行完当前指令后，立即响应，引发中断过程。
...
几乎所有由外设引发的外中断，都是可屏蔽中断。当外设由需要处理的事件发生时，相关芯片向CPU发出可屏蔽中断信息。不可屏蔽中断时在系统中有必须处理的紧急情况发生时用来通知CPU的中断信息。
...
按下一个键的时候，开关接通，该芯片就产生一个扫描码，扫描码说明了按下的键在键盘上的位置。扫描码被送入主板上的相关接口芯片的寄存器中，该寄存器的端口地址为60h。
...
<img src="Assembly.assets/image-20221028190638778.png" alt="image-20221028190638778" style="zoom:50%;" />

<img src="Assembly.assets/image-20221028190652935.png" alt="image-20221028190652935" style="zoom:50%;" />

#### 2. 引发9号中断

键盘的输入到达60h端口时，相关的芯片就会向CPU发出终端类型为9的可屏蔽中断信息。CPU检测到该中断信息后，如果IF = 1，则响应中断，引发中断过程，转区执行`int 9`中断例程。
...
1. 读出60h端口中的扫描码
2. 如果是字符键的扫描码，将该扫描码和它所对应的字符码（即ASCII码）送入内存中的BIOS键盘缓冲区；如果是控制键（比如Ctrl）和切换键（CapsLock）的扫描码，则将其转变为状态字节（用二进制位记录控制键和切换键状态的字节码）写入内存中存储状态字节的单元
3. 对键盘系统进行相关的控制，比如说，向相关芯片发出应答信息

BIOS键盘缓冲区是系统启动后，BIOS用于存放`int 9`中断例程所接受的键盘输入的内存区。该内存区可以存储15个键盘输入，因为`int 9`中断例程除了接受扫描码外，还要产生和扫描码对应的字符码，所以在BIOS键盘缓冲区中，一个键盘输入用一个字单元存放，高位字节存放扫描码，低位字节存放扫描码。

0040:17单元存储键盘状态字节，该字节记录了控制键和切换键的状态。键盘状态字节各位记录的信息如下：

<img src="Assembly.assets/image-20221028192243319.png" alt="image-20221028192243319" style="zoom:50%;" />
...
上面的过程中，前三步都是由硬件系统完成的。我们能够改变的只有`int 9`中断处理程序。我们可以重新编写`int 9`，按照自己的意图来处理键盘的输入。

******

如果我们要编写一个完整的硬件处理程序，需要对硬件的细节进行处理。由于BIOS提供的`int 9`中断例程已经对 硬件细节进行了处理，因此在这里，我们在自己的中断程序中调用BIOS的`int 9`中断。
...
<img src="Assembly.assets/image-20221028195012408.png" alt="image-20221028195012408" style="zoom: 33%;" />

在上面的程序的执行过程中，我们无法看清屏幕的显示。因为一个字母刚显示到屏幕上，CPU执行几条指令之后，就又变成了另外一个字母，字母切换的太快，我们无法看清。
